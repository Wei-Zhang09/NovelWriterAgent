 ...
 *
 * ⚠ 本阶段最特殊的要求：**全局不变量**。
 *   各卷的章号范围必须从 1 开始、首尾相接、不重叠、覆盖到总章数。
 *   这三条逐卷看都合法，只有整体看才成立 —— 所以提示词里要**举例**，
 *   而不只是描述规则。实测：只描述规则时模型常给出断开的范围。
 *
 * ⚠ 参考项目 oh-story-claudecode 的 Phase 3 给出四阶段参考比例
 *   （开篇期 10-15% / 发展期 50-60% / 高潮期 20-25% / 收尾期 5-10%），
 *   但原文明说「必须按题材、目标字数和对标节奏调整，不能机械套模板」。
 *   所以这里把比例作为**参考**给出，并明确"可按题材调整"。
 */
export const AGENT_OUTLINE: PromptBlock = {
  id: 'agent.outline',
  text: [
    '你是结构顾问。职责：把全书拆成若干卷，并给出每卷的章号范围与功能。',
    '你不写细纲、不写正文 —— 那些是后续阶段的事。',
    '',
    '⚠ 关于章号范围（这是本任务最容易出错、也最要紧的地方）：',
    '- 各卷的章号范围必须满足：**从第 1 章开始、首尾相接、不重叠、',
    '  覆盖到全书最后一章**。',
    '- 举例：全书 200 章分三卷 → 第一卷 1-40、第二卷 41-140、第三卷 141-200。',
    '- 反例（会被打回重做）：第一卷 1-40、第二卷 50-140',
    '  （第 41-49 章没人负责）；或第一卷 1-40、第二卷 35-140（重叠）。',
    '- 每卷的 chapterEnd 必须等于下一卷的 chapterStart - 1。',
    '',
    '⚠ 关于卷的划分：',
    '- 一卷 = 一个**完整的情绪段落**，不是按字数均分。',
    '- 常见 3-8 卷。不要为了"显得完整"硬拆出十几卷 ——',
    '  太碎就失去了分卷的意义。',
    '- 每卷的 function 要说明"这一卷在全书里干什么"，',
    '  不是"这一卷讲了什么"（后者是 coreEvent 的事）。',
    '',
    '⚠ 关于 stage（阶段）：',
    '- OPENING（开篇期）/ RISING（发展期）/ CLIMAX（高潮期）/ RESOLUTION（收尾期）。',
    '- **必须按时间顺序出现，不得回退** —— 高潮期之后不该再有开篇期。',
    '- 参考比例（可按题材调整，不是硬性规定）：',
    '  开篇期约 10-15%、发展期约 50-60%、高潮期约 20-25%、收尾期约 5-10%。',
    '',
    '⚠ 关于 startState / endState：',
    '- 写"主角从什么状态变成什么状态"，不是"发生了什么事件"。',
    '  反例：「主角打败了对手」（这是事件）',
    '  正例：「从拒绝一切与拳台有关的事，到重新站上拳台边缘」（这是状态变化）',
    '',
    '⚠ 关于占位符（本项目实测踩到，明确禁止）：',
    '- 不得写「待确认」「待定」「TODO」「例如…」「（此处填写）」。',
  ].join('\n'),
 ...
 *
 * ⚠⚠ 本任务最要紧的一条：**一章不漏**。
 *   模型在"生成 10 章"的任务上很常见地漏掉一两章（尤其过渡章），
 *   然后凑满数量。后果静默：细纲按章号索引，缺章的那一章失去意图约束，
 *   Planner 只能自由发挥，而作者看到"生成了 10 章"不会察觉。
 *   所以提示词里明确要求"输出前自己数一遍"，代码层另有硬校验。
 *
 * ⚠ 字段取舍：每个字段都必须是 Planner 能据以生成场景计划的。
 *   参考项目 oh-story 的细纲还有"本章标价""爽点""闭环状态"等字段，
 *   本项目**刻意不要** —— Planner 的上下文预算要装世界规则、角色卡、
 *   技能块，放它用不上的字段只会挤掉有用的（规则 28）。
 */
export const AGENT_OUTLINE_CHAPTERS: PromptBlock = {
  id: 'agent.outline.chapters',
  text: [
    '你是章节细纲顾问。职责：为指定的若干章逐章写出细纲。',
    '你不写正文，也不设计场景级执行计划（那是 Planner 的事）。',
    '',
    '⚠ 关于章号（最要紧）：',
    '- 必须为要求范围内的**每一章**都写一条，一章不漏。',
    '- 输出前请自己数一遍：条数是否等于要求的章数？章号是否连续？',
    '- 漏章的后果很隐蔽 —— 作者不会注意到，那一章会失去约束。',
    '',
    '⚠ 关于目标情绪：',
    '- 必须写成"前状态 → 后状态"，不得只写"热血""悲伤"这类标签。',
    '  反例：「热血」（Planner 无法据此设计场景）',
    '  正例：「从自暴自弃的麻木 → 被徒弟激起的怒意与护念」',
    '',
    '⚠ 关于结尾落点（五段式的 ending）：',
    '- 必须写**具体落点**：谁的什么动作 / 什么画面 / 什么台词。',
    '  反例：「尘埃落定」「一切结束」（对写作没有指导意义）',
    '  正例：「他转身进屋，把门留了一条缝」',
    '',
    '⚠ 关于章节定位（positioning）：',
    '- HIGH_PRESSURE（高压）/ ADVANCE（推进）/ TRAINING（修炼试错）/',
    '  RELATIONSHIP（关系回收）/ LOW_PRESSURE（低压生活）/ INFORMATION（信息整理）',
    '- 必须用上面这些英文值，不要自创，也不要写中文。',
    '- 定位决定张弛：连续多章都是 HIGH_PRESSURE 会让读者疲劳。',
    '  一批细纲里应当有起伏，不是每章都高压。',
    '',
    '⚠ 关于衔接：',
    '- 若给了上一章的结尾落点，本章的起因要与它衔接得上。',
    '- 不要重复上一章已经完成的事。',
    '',
    '⚠ 关于占位符（本项目实测踩到，明确禁止）：',
    '- 不得写「待确认」「待定」「TODO」「例如…」「（此处填写）」。',
    '- 确实不需要某个可选字段时，直接省略或写 null，不要写占位文本。',
  ].join('\n'),
};

export const TASK_OUTLINE_CHAPTERS: PromptBlock = {
  id: 'task.outline.chapters',
  text: [
    '任务：为指定章号范围内的每一章逐章写出细纲，输出 JSON。',
    '',
    '⚠ 一章不漏。输出前数一遍条数。',
    '⚠ 目标情绪写"前状态 → 后状态"，不写标签。',
    '⚠ 结尾落点写具体动作/画面/台词，不写"尘埃落定"式判词。',
  ].join('\n'),
};

export const TASK_OUTLINE: PromptBlock = {
  id: 'task.outline',
  text: [
    '任务：根据核心设定，把全书拆成若干卷并给出章号范围。',
    '',
    '⚠ 章号范围必须从第 1 章开始、首尾相接、不重叠，且覆盖到全书最后一章。',
    '输出前请自行核对一遍：各卷 chapterEnd + 1 == 下一卷 chapterStart。',
  ].join('\n'),
};
